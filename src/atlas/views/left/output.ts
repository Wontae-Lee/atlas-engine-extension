...
export class Output extends ProjectView {
	...
	protected items(state: ProjectState): ViewItem[] {
		...
		return [
			...
			this.action_row('Export Particle Snapshot CSV', { action: 'export', field: 'particles' }, 'export', available),
			this.action_row('Export Snapshot Statistics CSV', { action: 'export', field: 'statistics' }, 'export', available)
		];
	}
	...
	private async export_snapshot(api: typeof vscode, kind: string | undefined): Promise<void> {
		if (kind !== 'particles' && kind !== 'statistics') { throw new Error('Unsupported export type.'); }
		const snapshot = this.streaming.last_snapshot;
		if (!snapshot) { throw new Error('No simulation snapshot is available to export.'); }
		...
		if (kind === 'particles') {
			...
			if (statistics.species.length) {
				for (const species of statistics.species) { lines.push([...totals, species.index, species.count].join(',')); }
			} else { lines.push([...totals, '', ''].join(',')); }
		}
