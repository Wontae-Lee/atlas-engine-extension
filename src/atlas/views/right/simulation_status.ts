...
export class SimulationStatus extends View {
    ...
    getChildren(element?: vscode.TreeItem): ViewItem[] {
        ...
        const rows = [
            {id: 'state', label: 'State', description: this.streaming.state},
            {
                id: 'settings',
                label: 'Case settings',
                description: applied ? 'Applied to engine' : 'Not applied to engine'
            },
            ...
            {id: 'cells', label: 'Cells', description: snapshot ? String(snapshot.cell_count) : '—'},
            {
                id: 'mean-speed',
                label: 'Mean speed',
                description: this.statistics?.mean_speed === undefined ? '—' : `${this.statistics.mean_speed.toPrecision(6)} m/s`
            },
            {
                id: 'rms-speed',
                label: 'RMS speed',
                description: this.statistics?.rms_speed === undefined ? '—' : `${this.statistics.rms_speed.toPrecision(6)} m/s`
            },
    ...
    private record_snapshot(): void {
        ...
        if (previous?.step === sample.step && previous.time === sample.time) {
