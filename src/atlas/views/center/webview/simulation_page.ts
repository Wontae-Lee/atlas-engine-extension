...
export class SimulationPage {
	...
	constructor(on_frame: (message: unknown) => void) {
		window.addEventListener('message', event => {
			if (event.data?.type === 'error') { this.show_error(String(event.data.message)); }
			else { on_frame(event.data); }
		}, { signal: this.abort.signal });
		for (const button of document.querySelectorAll<HTMLButtonElement>('button[data-action]')) {
			button.addEventListener('click', () => {
				for (const control of document.querySelectorAll<HTMLButtonElement>('button[data-action]')) { control.disabled = true; }
	...
	send(message: unknown): void {
		if (typeof message === 'object' && message !== null && 'type' in message
			&& message.type === 'action') { this.show_error(''); }
