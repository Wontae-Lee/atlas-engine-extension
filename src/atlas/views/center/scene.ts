	...
	protected async data(): Promise<SceneFrame> {
		...
		if (this.asset_revision !== status.revision) {
			...
			this.assets = Promise.all(project.assets.filter(asset => referenced.has(asset.id)).map(async asset => {
				try { return { id: asset.id, content: await this.project.assets.read_asset(asset) }; }
				catch (error) { return { id: asset.id, error: `${asset.name}: ${error instanceof Error ? error.message : String(error)}` }; }
			}));
		}
