...
import {type BackendConnection, type BackendInfo, type BackendMode, IMAGES} from '../backend/backend_types';
...
export class ContainerConnection implements BackendConnection {
    ...
    constructor(
        ...
        args.push('--entrypoint', 'python', IMAGES[mode], '-u', '-c', runtime_source);
    ...
    async info(signal?: AbortSignal): Promise<BackendInfo> {
        const result = await this.request('info', undefined, signal) as Partial<BackendInfo> | null;
    ...
    request(method: string, params?: unknown, signal?: AbortSignal): Promise<unknown> {
        return this.requests.request(method, params, signal);
    }
    ...
    private remove_container(): void {
        void run(this.executable, ['rm', '--force', this.name], undefined, () => {
        }, 10_000).catch(error => {
