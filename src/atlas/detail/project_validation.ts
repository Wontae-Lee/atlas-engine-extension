...
export function validate_project(state: ProjectState, for_engine = false): void {
    ...
    for (const section of ['assets', 'materials', 'geometry', 'sources', 'boundaries', 'sinks'] as const) {
        ...
        for (const value of entries) {
            ...
            all_ids.add(id);
            if (section === 'assets') {
                references.asset.add(id);
            }
            if (section === 'geometry') {
                references.geometry.add(id);
            }
            if (section === 'materials') {
                references.material.add(id);
            }
        }
    }
    ...
    numeric(solver.majorant_sample_pairs, 'DSMC solver: Majorant sample pairs', {
        integer: true,
        min: 1,
        max: 2147483647
    });
    numeric(solver.majorant_exhaustive_limit, 'DSMC solver: Majorant exhaustive limit', {
        integer: true,
        min: 2,
        max: 2147483647
    });
    ...
    for (const entry of state.geometry) {
        geometry(entry, references);
    }
    for (const entry of state.sources) {
        sources(entry, references);
    }
    for (const entry of state.boundaries) {
        boundaries(entry, references);
    }
    for (const entry of state.sinks) {
        sinks(entry, references);
    }
