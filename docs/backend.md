...
확장이 `onStartupFinished`로 활성화되면 아래 상태 표시줄에 `atlas-engine-backend`가 표시된다.
...
저장된 CUDA 선택이 있으면 CUDA로 연결을 시도한다. 활성화 로그가 출력되더라도
비동기 연결은 진행 중일 수 있으므로 상태 표시줄과 Output으로 결과를 확인한다.
...
새 백엔드를 준비하는 동안 취소하거나 준비·검증이 실패하면 기존 연결을 유지한다.
...
연결이 없으면 현재 선택 모드로 먼저 연결한다. 연결·검사 작업 중에는 다른 선택·검사 요청을 시작하지 않는다.
진행 중인 검사를 취소하면 그 요청을 실행하던 컨테이너 연결도 종료되므로 다시 연결해야 한다.
...
이후 snake_case 정리, `src/atlas/` 이동과 `detail/private_helpers.ts` 통합 후에는
빌드·테스트를 다시 실행하지 않았다. 아래 기록은 현재 변경본의 검증 결과가 아니다.
